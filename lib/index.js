...
class App {
  ...
  constructor() {
    console.log();
    console.log(
      `${gradientString.pastel("----------------------")} ${chalk.white.bold(
        "aastbis"
      )} ${chalk.white.bold(packageJson.version)} ${gradientString.pastel(
        "----------------------"
      )}`
    );
    console.log();
  ...
  setConfig() {
    ...
    const args = require("yargs")
      ...
      .option("server", {
        alias: "s",
        describe: "Start aastbis has server",
        default: false,
        type: "boolean",
      })
      .option("host", {
    ...
    // Custom
    config.domain = `http://${config.host ? config.host : ip.address()}${
    ...
    if (this.config.debug >= 1) {
      for (const _configKey in this.config) {
        ...
        console.log(
          `${chalk.green.bold("app > config")} - ${chalk.cyan(
            _configKey
          )} - ${chalk.cyan(_configValue)}`
        );
      }
    }
  }
  ...
  setServer() {
    this.setSite(
      ...
      () => {
        ...
        console.log(
          `${chalk.green.bold("app")} - ${chalk.cyan(
            "aastbis seems to be running already"
          )}`
        );
      }
    );
  ...
  setSocket() {
    ...
    watcherSocket.on("connection", (socket) => {
      ...
      if (this.config.debug >= 1) {
        console.log(
          `${chalk.green.bold("app > socket")} - ${chalk.cyan(
            "connect"
          )} - ${chalk.cyan(socket.id)}`
        );
      }
      ...
      callbacks.startProject = (data) => {
        ...
        if (this.config.debug >= 2) {
          console.log(
            util.inspect(project.files.describe(), {
              depth: null,
              colors: true,
            })
          );
