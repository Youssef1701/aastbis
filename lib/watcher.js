...
class Watcher {
  ...
  setSocket() {
    ...
    this.socket.on("connect", () => {
      ...
      if (this.config.debug) {
        console.log(
          `${chalk.green.bold("watcher > socket")} - ${chalk.cyan("connect")}`
        );
      }
    });
    ...
    this.socket.on("disconnect", () => {
      ...
      if (this.config.debug) {
        console.log(
          `${chalk.green.bold("watcher > socket")} - ${chalk.cyan(
            "disconnect"
          )}`
        );
      }
    ...
    this.socket.on("started", () => {
      ...
      console.log(
        `${chalk.green.bold("app")} - ${chalk.yellow.underline(url)}`
      );
    ...
    this.socket.on("alert", () => {
      nodeNotifier.notify({
        ...
        icon: path.join(
          __dirname,
          "assets/images/notification-icon-200x200.png"
        ),
        wait: true,
      });
    });
  ...
  watch() {
    const globPattern = [
      "**",
      ...this.config.exclude.map((item) => `!${item}`),
    ];
    ...
    if (ignoreInitial) {
      console.log(
        `${chalk.green.bold("watcher")} - ${chalk.cyan(
          "start wating"
        )} - ${chalk.red(`File limit exceeded (${this.config.limit})`)}`
      );
    }
    ...
    this.watcher.on("add", (_path) => {
      ...
      if (
        mimeType &&
        mimeType.match(/^(audio)|(video)|(image)/) &&
        extension !== ".ts"
      ) {
        file.canRead = false;
      }
      ...
      fs.stat(_path, (error, stats) => {
        ...
        if (error) {
          console.log(
            `${chalk.green.bold("watcher")} - ${chalk.cyan(
              "add"
            )} - ${chalk.cyan(_path)} - ${chalk.red("error")}`
          );
          return;
        }
      ...
      if (this.config.debug >= 1) {
        console.log(
          `${chalk.green.bold("watcher")} - ${chalk.cyan("add")} - ${chalk.cyan(
            relativePath
          )}`
        );
      }
